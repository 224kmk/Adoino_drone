...
void softmax_Y(double *Y, int N) {
	double YMax = Y[0];
	for(int n=1;n<N;n++) if(Y[n]>YMax) YMax = Y[n];
	for(int n=0;n<N;n++) Y[n] -= YMax;
	double sumY = 0;
	for(int n=0;n<N;n++) sumY += exp(Y[n]);
	for(int n=0;n<N;n++) Y[n] = exp(Y[n])/sumY;
}

double calculate_CEE(double *Y, double *T, int N) {	
	double E = 0.0;
	for(int n=0;n<N;n++) E += -T[n]*log(Y[n]);
	return E;
}
...
void initialize_weight_He(double *W, int M, int N) {
	for(int m=0;m<M;m++) 
		for(int n=0;n<N;n++) 
			W[m*N+n] = randn()/sqrt(M/2.0); // He	
}	

void initialize_weight_Le(double *W, int M, int N) {
	for(int m=0;m<M;m++) 
		for(int n=0;n<N;n++) 
			W[m*N+n] = randn()/sqrt(M); // Lecun	
}